...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        evaluated_expr = scheme_eval(expressions.second.first, env)
        env.define(target, evaluated_expr)
        return target
        # END PROBLEM 6
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 13
    # do not check form since we removed and at this point so 0 is valid
    if expressions is nil:
        return True
    else:
        evaluated_first = scheme_eval(expressions.first, env)
        if scheme_truep(evaluated_first):
...
def do_or_form(expressions, env):
    ...
    evaluated_first = scheme_eval(expressions.first, env)
    if type(evaluated_first) == type(0) and evaluated_first == 0:
        return evaluated_first
    elif evaluated_first != False:
        return evaluated_first
    else:
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            # BEGIN PROBLEM 14
            if clause.second == nil: #if the predicate doesn't have a corresponding expression return the predicate
                return test
            else:
